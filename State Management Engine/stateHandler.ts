...
export class StateHandler {
  ...
  public async execute(
    eventType: string,
    eventPayload?: any                     // payload type now any
  ): Promise<boolean> {                    
    ...
    /* ---------- run validation ---------- */
    if (validateFn) {
      const ok = await validateFn(event, this._context);   // ⬅ await handles sync & async
      if (!ok) {
    ...
    if (actions) {
      for (const action of actions) {
        try {
          await action(event, this._context);              // ⬅ await handles sync & async
        } catch (err) {
